...
- **全局快捷键**：默认 `⌃⌥⌘M`，用于切换内建显示器，可在 Settings 中修改。
...
为每个使用地点各建一个 Profile——例如“办公室”关闭一台不常用的副屏，“家里”启用它。从状态栏的**配置档**子菜单切换。Profile 不会根据地点或显示器拓扑自动切换，选择权始终在你手里。

**单屏专注模式**

建一个 Profile，用高优先级的 `always` 规则按显示器身份（或显示器族）关闭外接屏。需要专注时从状态栏一键激活，结束后再切回。内建屏受最后活动屏保护，规则不可能让你陷入无屏可用的境地。

**热键快速切换内建屏**

默认 `⌃⌥⌘M` 切换内建屏的启停。双屏桌面和纯外接屏模式之间来回切换无需打开设置。快捷键被其他应用占用时，可在 Settings 中改绑。

**显示器变化后的自动收敛**

数量条件监控外接屏的在线/活动数量。例如：规则仅在外接活动屏 ≥ 2 台时启用某台投影显示器，数量下降后停用它。自动化在启动/唤醒稳定期之后、以及轮询间隔内求值，插拔线缆后状态会自行收敛。这类规则要分配不同的优先级——同优先级规则对同一显示器请求冲突动作时会被报告为冲突，而不是直接执行。

**演示 / 投影场景**

建一个专用“演示”Profile，规则按显示器身份或显示器族启用投影仪。动作冲突时最高优先级规则胜出。激活前会预览最新拓扑，涉及关闭显示器时要求确认，提交前可以先核对将要发生的变化。

**意外退出后的恢复**

应用关闭某台显示器后、操作尚未确认完成时退出或崩溃，恢复证据会保留在 `runtime-state.json` 中。下次启动时用状态栏的恢复动作把受管显示器找回来。恢复、自动化和手动操作在每一步都满足最后活动屏安全约束。

以上场景都只改变显示器的启用/停用状态，不会触碰排列、分辨率、刷新率或色彩配置。
